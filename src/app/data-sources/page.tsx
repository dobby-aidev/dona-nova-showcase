...
import { Database, ExternalLink, ShieldCheck, CheckCircle2, Globe2 } from "lucide-react";
      ...
      <main className="flex-1 flex flex-col items-center justify-start px-4 sm:px-6 py-10 sm:py-14 w-full max-w-3xl mx-auto text-center">
        ...
        <div className="w-full text-center mb-10 space-y-4">
          ...
          <p className="text-xs sm:text-sm text-zinc-400 max-w-lg mx-auto leading-relaxed">
            DONA NOVA üzerindeki hiçbir veri tahmini veya uydurma değildir. Her tesis ve şebeke düğümü kamuya açık resmi kaynaklardan doğrulanır.
          </p>

          <div className="flex flex-wrap items-center justify-center gap-2.5 text-[11px] font-mono text-zinc-500 pt-1">
            <span className="rounded-md border border-white/10 bg-white/[0.04] px-2.5 py-1 text-zinc-300">3.160+ Haritalanmış Tesis</span>
            <span className="text-zinc-600">•</span>
            <span className="rounded-md border border-white/10 bg-white/[0.04] px-2.5 py-1 text-zinc-300">Sıfır Uydurma Veri</span>
            <span className="text-zinc-600">•</span>
            <span className="rounded-md border border-cyan-500/30 bg-cyan-950/40 text-cyan-300 font-bold px-2.5 py-1">Resmi Açık API</span>
          </div>
        </div>

        {/* 3 Metodoloji İlkesi - Centered Cards */}
        <div className="w-full grid grid-cols-1 sm:grid-cols-3 gap-3.5 mb-10 text-center">
          <div className="rounded-2xl border border-white/10 bg-zinc-950/60 p-5 space-y-2 backdrop-blur-xl shadow-xl flex flex-col items-center">
            <div className="flex h-7 w-7 items-center justify-center rounded-xl bg-cyan-950/80 border border-cyan-500/40 text-cyan-300 font-mono text-xs font-black shadow-[0_0_10px_rgba(6,182,212,0.2)]">
              1
            </div>
            ...
            <p className="text-[11px] text-zinc-400 leading-relaxed text-center">
              Aracı şirket kullanılmaz. Veriler WRI, EIA ve ENTSO-E kamu kurumlarından doğrudan çekilir.
            </p>
          </div>

          <div className="rounded-2xl border border-white/10 bg-zinc-950/60 p-5 space-y-2 backdrop-blur-xl shadow-xl flex flex-col items-center">
            <div className="flex h-7 w-7 items-center justify-center rounded-xl bg-amber-950/80 border border-amber-500/40 text-amber-300 font-mono text-xs font-black shadow-[0_0_10px_rgba(245,158,11,0.2)]">
              2
            </div>
            <h3 className="text-xs font-bold text-white font-sans">Standart Normalizasyon</h3>
            <p className="text-[11px] text-zinc-400 leading-relaxed text-center">
              Farklı birimler (MW, GW) ortak altyapı veri şemasına dönüştürülerek haritada birleştirilir.
            </p>
          </div>

          <div className="rounded-2xl border border-white/10 bg-zinc-950/60 p-5 space-y-2 backdrop-blur-xl shadow-xl flex flex-col items-center">
            <div className="flex h-7 w-7 items-center justify-center rounded-xl bg-emerald-950/80 border border-emerald-500/40 text-emerald-300 font-mono text-xs font-black shadow-[0_0_10px_rgba(16,185,129,0.2)]">
              3
            </div>
            <h3 className="text-xs font-bold text-white font-sans">Açık Kaynak Kodları</h3>
            <p className="text-[11px] text-zinc-400 leading-relaxed text-center">
              Tüm veri işleme komut dosyaları GitHub deposunda açık kaynak olarak incelenebilir.
            </p>
          </div>
        </div>

        {/* Kaynak Kartları - Centered Layout */}
        <div className="w-full space-y-4">
          {SOURCES.map((source) => (
            <div
              key={source.id}
              className="rounded-2xl border border-white/10 bg-zinc-950/70 p-6 shadow-2xl backdrop-blur-2xl space-y-4 transition-all hover:border-cyan-500/40 hover:bg-zinc-900/80 text-center"
            >
              <div className="flex flex-col items-center justify-center gap-2 border-b border-white/[0.08] pb-4">
                <div className="flex items-center justify-center gap-2">
                  <span className="rounded-md bg-cyan-950/60 border border-cyan-500/30 px-2.5 py-0.5 text-[10px] font-bold font-mono text-cyan-300">
                    {source.category}
                  </span>
                ...
                <a
                  ...
                  className="inline-flex items-center gap-1.5 text-xs font-mono font-bold text-cyan-400 hover:text-cyan-300 hover:underline pt-0.5"
                >
                  <span>Resmi Kaynağı İncele</span>
                  <ExternalLink className="h-3 w-3" />
                </a>
              </div>

              <p className="text-xs sm:text-sm text-zinc-300 leading-relaxed max-w-xl mx-auto text-center">
                {source.desc}
              </p>

              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 rounded-xl bg-black/40 border border-white/[0.06] p-3 text-xs font-mono text-center">
                <div>
        ...
        <div className="text-center pt-14 pb-8 text-xs font-mono text-zinc-500">
          <p>DONA NOVA • Verified Infrastructure Dataset • Dona Codex</p>
        </div>
      </main>
    </div>
  );
